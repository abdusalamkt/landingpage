...
export default function DownloadsPage() {
  ...
  const parsedDownloads = (data?.downloads?.nodes || []).flatMap((node) => {
    const productTitle = node.downloadFields?.product?.title || "";
    return node.downloadFields?.productdownloads?.map((item) => ({
  ...
  const uniqueValues = (key) => [
    ...new Set(parsedDownloads.map((item) => item[key])),
  ].filter(Boolean);
  ...
  if (error) return <p>Error loading downloads.</p>;
