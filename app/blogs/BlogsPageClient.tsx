...
export default function BlogsPage() {
  ...
  const filteredPosts = posts
    .filter((post) => post.blogPostFields.categoryLabel !== 'Press Release')
    .filter((post) => {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "blogPost": posts.map((p) => ({
              ...
              "url": `https://gfiuae.com/blogs/${p.slug}`,
              "author": {
                "@type": "Organization",
                "name": "GFI UAE"
              },
              "image": p.featuredImage?.node?.sourceUrl || '/placeholder.jpg',
              "keywords": Array.isArray(p.blogPostFields.productTags)
                ? p.blogPostFields.productTags
                : p.blogPostFields.productTags?.split(',').map((t: string) => t.trim()) || []
            }))
          })
        }}
      />
