...
import PrimeraClientFeatures from "./PrimeraClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/primera";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Primera page");
      return mapSEOtoMetadata(data.seo, fallbackUrl);
    }

    // Generate from page fields if SEO not available
    if (data?.washroomCubiclesField) {
      const { heading, description, heroImage } = data.washroomCubiclesField;
      const title = `${heading || "Primera Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium Primera washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Primera page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Primera Washroom Cubicles | GFI UAE",
    description: "Premium Primera washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}

// ✅ Main Server Component
export default async function PrimeraPage() {
  const data = await getWashroomCubiclesData();
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The Primera washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <PrimeraClientFeatures acfData={data.washroomCubiclesField} />;
}