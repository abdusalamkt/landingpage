...
import GlassCubicleClientFeatures from "./GlassCubicleClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/glass-cubicle";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Glass Cubicle page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "Glass Cubicle"} | GFI UAE`;
      const desc =
        description ||
        "Premium glass washroom cubicles with customizable finishes and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Glass Cubicle page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Glass Cubicle | GFI UAE",
    description: "Premium glass washroom cubicles with customizable finishes and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}

// ✅ Main Server Component
export default async function GlassCubiclePage() {
  const data = await getWashroomCubiclesData();
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The glass cubicle page could not be loaded.</p>
      </div>
    );
  }

  return <GlassCubicleClientFeatures acfData={data.washroomCubiclesField} />;
}