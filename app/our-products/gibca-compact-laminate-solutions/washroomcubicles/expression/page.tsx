...
import ExpressionClientFeatures from "./ExpressionClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/expression";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Expression page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "Expression Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium Expression washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Expression page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Expression Washroom Cubicles | GFI UAE",
    description: "Premium Expression washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The Expression washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <ExpressionClientFeatures acfData={data.washroomCubiclesField} />;
}