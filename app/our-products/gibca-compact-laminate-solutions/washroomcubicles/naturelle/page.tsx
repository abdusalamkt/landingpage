...
import NaturelleClientFeatures from "./NaturelleClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/naturelle";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Naturelle page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "Naturelle Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium Naturelle washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Naturelle page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Naturelle Washroom Cubicles | GFI UAE",
    description: "Premium Naturelle washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}

// ✅ Main Server Component
export default async function NaturellePage() {
  const data = await getWashroomCubiclesData();
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The Naturelle washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <NaturelleClientFeatures acfData={data.washroomCubiclesField} />;
}