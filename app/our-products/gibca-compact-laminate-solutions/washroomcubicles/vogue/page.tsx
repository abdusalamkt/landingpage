...
import VogueClientFeatures from "./VogueClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/vogue";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for vogue page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "vogue Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium vogue washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for vogue page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "vogue Washroom Cubicles | GFI UAE",
    description: "Premium vogue washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}

// ✅ Main Server Component
export default async function VoguePage() {
  const data = await getWashroomCubiclesData();
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The vogue washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <VogueClientFeatures acfData={data.washroomCubiclesField} />;
}