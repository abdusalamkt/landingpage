...
import UnoClientFeatures from "./UnoClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/uno";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Uno page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "Uno Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium Uno washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Uno page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Uno Washroom Cubicles | GFI UAE",
    description: "Premium Uno washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The Uno washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <UnoClientFeatures acfData={data.washroomCubiclesField} />;
}