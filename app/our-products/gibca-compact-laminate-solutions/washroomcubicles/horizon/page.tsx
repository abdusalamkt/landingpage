...
import HorizonClientFeatures from "./HorizonClientFeatures";
...
export async function generateMetadata(): Promise<Metadata> {
  const fallbackUrl = "https://gfiuae.com/horizon";
  ...
  try {
    ...
    if (data?.seo) {
      console.log("✅ SEO found for Horizon page");
    ...
    if (data?.washroomCubiclesField) {
      ...
      const title = `${heading || "Horizon Washroom Cubicles"} | GFI UAE`;
      const desc =
        description ||
        "Premium Horizon washroom cubicles with customizable finishes, patterns, and smart features. Explore our range at GFI UAE.";
    ...
    console.warn("⚠️ No SEO or page data found for Horizon page, using fallback");
  } catch (error) {
  ...
  // Generic fallback
  return {
    title: "Horizon Washroom Cubicles | GFI UAE",
    description: "Premium Horizon washroom cubicles with customizable finishes, patterns, and smart features from GFI UAE.",
    alternates: { canonical: fallbackUrl },
  };
}
  ...
  if (!data?.washroomCubiclesField) {
    return (
      <div style={{ padding: "50px", textAlign: "center" }}>
        ...
        <p>The Horizon washroom cubicles page could not be loaded.</p>
      </div>
    );
  }

  return <HorizonClientFeatures acfData={data.washroomCubiclesField} />;
}